...
function build_bar(pIdx, pSumCnt, pSize, show) {
    ...
    var upPage = cond(isUpPageHide, "", concat(["<li class='prev'><a  pg='"+(pIdx - 1)+"' href='javascript:{}'><</a></li>"]))
    //是否隐藏下一页
    var isDownPageHide = pIdx >= end_idx
    var downPage = cond(isDownPageHide, "", concat(["<li class='next'><a  pg='"+(pIdx + 1)+"' href='javascript:{}'>></a></li>"]))
    //是否隐藏末页
    ...
    for (var i = min;i<=max;i++) {
        if (i != pIdx) {
            ...
            mmArr[idx] = concat(["<li ><a class='selected' href='javascript:{}' pg='"+i+"' >", i, "</a></li>"])
        }
        idx = idx + 1
    }
    ...
    //第一页
    var pIdx1Str = cond(pIdx == 1, "<li ><a class='selected' href='javascript:{}' pg='1' >1</a></li>", "<li class=''><a href='javascript:{}'  pg='1' >1</a></li>")
    var currPageStr = cond(find(show, "p") , concat(["<li><a>第", pIdx, "页</a></li>"]), "")
    var pageStr = cond(find(show, "s") , concat(["<li><a>共", end_idx, "页</a></li>"]), "")
    var sumPageStr = cond(find(show, "a") , cond(end_idx > 0, concat([pageStr, "<li><a>共", pSumCnt, "条</a></li>"]), "<li><a>暂无数据</a></li>"), "")
