...
function canBuy(){
	...
	$(".btn_box .addtocart-btn").unbind('click');
	$(".btn_box .addtocart-btn").bind('click',function(){
		Goods.addToCart($(this));
		return false;
	});
	...
	$(".btn_box .addtocart-btn").css("cursor","pointer");
	$(".btn_box .addtocart-btn").tip({'disable':true});
...
var BtnTipRefresh = {
	refresh:function(pro){
		$(".buynow-btn").attr('tip','');
		$(".gnotifybtn").attr('tip','');
		$(".btn_box .addtocart-btn").attr('tip','');
		if(pro.length==1){
			if(pro[0].store==0){
				...
				$(".gnotifybtn").show();
				$(".buynow-btn").hide();
				$(".btn_box .addtocart-btn").hide();
				
			}else{
				canBuy();
				$(".gnotifybtn").hide();
				$(".buynow-btn").show();
				$(".btn_box .addtocart-btn").show();
			}
		}else{
			$(".buynow-btn").unbind('click');
			...
			$(".btn_box .addtocart-btn").unbind('click');
			$(".btn_box .addtocart-btn").bind('click',function(){return false;});
			$(".btn_box .addtocart-btn").css("cursor","not-allowed");
			...
			$(".buynow-btn,.btn_box .addtocart-btn").tip({'disable':false,className:"cantbuy",text:"请选择:"+tip});
		}
	}	
};
