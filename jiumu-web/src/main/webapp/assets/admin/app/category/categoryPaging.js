...
function deleteItem(id){
    ...
    $.getJSON("admin/category/delete/"+id,{date:new Date().getTime()},function(data){
        if(data.code =='ok'){
            alert("删除成功")
            $("#tr"+id).remove()
        }else{
...
function batchDeleteItem(){
    ...
    $.getJSON("admin/category/batchdelete/"+ids.join(","),{date:new Date().getTime()},function(data){
        if(data.code =='ok'){
            alert("删除成功")
...
function  getItemPage(pageIndex){
    ...
    '<td>{name}</td><td>{sortNum}</td>' +
    '<td><a target="_blank" href="admin/category/detail/{id}">编辑</a> <a href="javascript:{}" onclick="deleteItem({id})">删除</a></td></tr>';
    var pageSize = 20;//每页多少条记录
    var pageCount = parseInt($("#pageCount").val());//总共多少条记录
    var url ='admin/category/page/'+pageIndex+'/'+pageCount;
