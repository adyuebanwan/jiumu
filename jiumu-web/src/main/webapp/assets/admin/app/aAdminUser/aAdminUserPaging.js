...
function deleteAAdminUser(id){
    ...
    $.getJSON("admin/aAdminUser/delete/"+id,{date:new Date().getTime()},function(data){
        if(data.code =='ok'){
            alert("删除成功")
            $("#tr"+id).remove()
        }else{
...
function  getAAdminUserPage(pageIndex){
    var template = '<tr id="tr{id}"><td>{rankNum}</td><td>{userName}</td><td>{realName}</td><td>{email}</td><td>{telephone}</td><td>{mobilePhone}</td>' + 
        '<td><a href="admin/aAdminUser/detail/{id}">编辑</a> <a href="javascript:{}" onclick="deleteAAdminUser({id})">删除</a></td></tr>';
    var pageSize = 20;//每页多少条记录
    var pageCount = parseInt($("#pageCount").val());//总共多少条记录
    var url ='admin/aAdminUser/page/'+pageIndex+'/'+pageCount;
